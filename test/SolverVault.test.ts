...
import { MockERC20, OnChainSymmioVault, SymmioVaultLpToken } from "../typechain-types"
...
describe("SymmioSolverDepositor", function () {
	...
	beforeEach(async function () {
		...
		const SymmioSolverDepositor = await ethers.getContractFactory("OnChainSymmioVault")
	...
	describe("initialize", function () {
		it("should revert if initialize second time", async() => {
			await expect(symmioDepositor.initialize(
				await symmio.getAddress(),
				await lpToken.getAddress(),
				await solver.getAddress(),
				500000000000000000n, // 0.5,
	...
	describe("deposit", function () {
		...
		it("should deposit tokens", async function () {
			let depositTx = await symmioDepositor.connect(user).deposit(depositAmount);
			...
			expect(await collateralToken.balanceOf(await symmioDepositor.getAddress())).to.equal(0)
		...
		it("should fail to deposit more than user limit", async function () {
			...
			await expect(symmioDepositor.connect(user).deposit(500n + 1n))
				.to.be.revertedWith("SymmioSolverDepositor: Deposit per user limit reached")
		})
			...
			describe("claimForWithdrawRequest", function () {
				...
				it("should fail if not pass 7 days", async function () {
					await expect(symmioDepositor.connect(receiver).claimForWithdrawRequest(requestId))
						.to.be.revertedWith("SymmioSolverDepositor: Request not pass withdrawal period")
				})
