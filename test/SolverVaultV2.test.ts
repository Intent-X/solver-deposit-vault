...
describe("OnChainSymmioVaultV2", function () {
  ...
  describe("initialize", function () {
    ...
    it("Should update deposit limit", async () => {
      ...
      await expect(symmioDepositor.connect(other).setDepositLimit(1000))
        .to.be.reverted;
    });

    it("Should update withdrawalPeriod", async () => {
      expect(await symmioDepositor.withdrawalPeriod()).to.be.eq(0);
      await expect(
  ...
  describe("deposit", function () {
    ...
    it("should update the current deposit amount", async function () {
      ...
      await symmioDepositor
        .connect(balancer)
        .acceptWithdrawRequest(0, [0], [depositAmount]);
    ...
    describe("acceptWithdrawRequest", function () {
      ...
      it("should fail with insufficient contract balance", async () => {
        await expect(
          symmioDepositor
            .connect(balancer)
            .acceptWithdrawRequest(0, requestIds, [withdrawAmount])
        ).to.be.revertedWith(
      ...
      it("should accept withdraw request with provided amount", async function () {
        ...
        await expect(
          symmioDepositor
            .connect(balancer)
            .acceptWithdrawRequest(depositAmount, requestIds, [withdrawAmount])
        )
          .to.emit(symmioDepositor, "WithdrawRequestAcceptedEvent")
      ...
      describe("claimForWithdrawRequest", function () {
        ...
        beforeEach(async function () {
          ...
          await symmioDepositor
            .connect(owner)
            .setWithdrawalPeriod(604800);
          
          await symmioDepositor
            .connect(balancer)
            .acceptWithdrawRequest(0, requestIds, [withdrawAmount]);
          lockedBalance =
            (await symmioDepositor.withdrawRequests(0)).acceptedAmount;
        });
  ...
  describe("withdrawNotLockedCollateralTokens", function () {
    ...
    it("should fail if trying to withdraw locked balance", async function () {
      ...
      await symmioDepositor
        .connect(balancer)
        .acceptWithdrawRequest(0, [0], [minAmountOut]);
  ...
  describe("Multiple withdraw requests", function () {
    ...
    it("should accept multiple requests at once", async function () {
      ...
      await expect(
        symmioDepositor
          .connect(balancer)
          .acceptWithdrawRequest(0, [0, 1], [withdrawAmount1,withdrawAmount2])
      )
        .to.emit(symmioDepositor, "WithdrawRequestAcceptedEvent")
